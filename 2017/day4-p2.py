...
def isValid(passphrase):
	...
	valid = True
	for underTest in enumerate(words[:-1]):
		for w in words[underTest[0]+1:]:
			if len(w) == len(underTest[1]):
				abc = list(w)
				for c1 in underTest[1]:
					for c2 in enumerate(abc):
						if c2[1] == c1:
							abc.pop(c2[0])
							break
				if len(abc) == 0:
...
def main():
	...
	print(sum([isValid(pp) for pp in INPUT.split('\n')]))
