...
INPUT="""dihjv (2158) -> gausx, ncdmp, hozgrub
...
class Program:
	def __init__(self, inputString):
		...
		moarParts = stringParts[0].split()
		self.parent = None
		self.name = moarParts[0]
		self.weight = int(moarParts[1][1:-1])
		self.towerWeight = 0
		self.children = []
		self.childrenNames = stringParts[1].split(", ") if len(stringParts) > 1 else []

	def __repr__(self):
		return f"({self.name} ({self.weight}) {self.children})"

def calculateTowerWeight(programs, node):
	for c in programs[node].children:
		calculateTowerWeight(programs, c.name)
	tW = 0
	for c in programs[node].children:
		tW += c.towerWeight + c.weight
	programs[node].towerWeight = tW

def findProblematic(programs, node):
	weights = [c.towerWeight + c.weight for c in programs[node].children]
	weightCounts = {w: 0 for w in weights}
	for w in weights:
		weightCounts[w] += 1
	probWeight = -1
	for w, wC in weightCounts.items():
		if wC == 1:
			probWeight = w
			break
	if probWeight < 0:
		return programs[node].name
	return findProblematic(programs, programs[node].children[weights.index(probWeight)].name)

def correctWeight(programs, node):
	wrongNode = programs[node]
	sibling = [s for s in wrongNode.parent.children if s.name != node][0]
	sibW = sibling.weight + sibling.towerWeight
	return sibW - wrongNode.towerWeight

def solve(input):
	progList = [Program(l) for l in input.split('\n')]
	programs = {p.name: p for p in progList}
	for p in programs:
		if len(programs[p].childrenNames) != 0:
			for c in programs[p].childrenNames:
				programs[p].children.append(programs[c])
				programs[c].parent = programs[p]
	root = None
	for p in programs:
		if programs[p].parent == None:
			root = p
			break
	calculateTowerWeight(programs, root)
	return correctWeight(programs, findProblematic(programs, root))
